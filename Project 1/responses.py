...
patterns = {
    ...
    "bye": r"\b(bye|goodbye|farewell|see\s+you)\b",
    "help": r"\b(help|assist|support|guide)\b",

    # Java concept patterns
    "if_statement": r"\b(if\s+statement|conditional|if\s+condition|if\s+then)\b",
    "for_loop": r"\b(for\s+loop|for\s+statement|for\s+iteration)\b",
    "variable": r"\b(variable|var|variables|value\s+storage)\b",
    "else_statement": r"\b(else\s+statement|else\s+condition|else\s+block)\b",
    "while_loop": r"\b(while\s+loop|while\s+statement|do\s+while)\b",
    "switch_statement": r"\b(switch\s+statement|switch\s+case|case\s+statement)\b",
    "method": r"\b(method|function|procedure|subroutine)\b",
    "array": r"\b(array|arrays|list\s+of\s+data|data\s+structure)\b",
    "string": r"\b(string|text|character\s+sequence)\b",
    "int": r"\b(int|integer|whole\s+number)\b",
    "double": r"\b(double|float|floating\s+point|decimal)\b",
    "access_modifier": r"\b(access\s+modifier|public|private|protected|access\s+control)\b",
    "return_type": r"\b(return\s+type|return\s+value|method\s+return)\b",
    "parameter": r"\b(parameter|argument|input|param)\b",
    "constructor": r"\b(constructor|initializer|new\s+object|instantiation)\b",
    "object": r"\b(object|instance|class\s+instance)\b",
    "class": r"\b(class|blueprint|object\s+template)\b",
    "inheritance": r"\b(inheritance|extends|subclass|parent\s+class|child\s+class)\b",
    "interface": r"\b(interface|contract|abstract\s+type)\b",
    "package": r"\b(package|namespace|code\s+organization)\b",
    "import": r"\b(import|include|using)\b",
    "exception": r"\b(exception|error\s+handling|error\s+catch|runtime\s+error)\b",
    "try_catch": r"\b(try\s+catch|exception\s+handling|catch\s+block|try\s+block)\b",
    "finally": r"\b(finally\s+block|cleanup|resource\s+release)\b",
    "overloading": r"\b(overloading|same\s+name\s+different\s+parameters|method\s+overload)\b",
    "overriding": r"\b(overriding|override|reimplement|polymorphism)\b",
    "static": r"\b(static|class\s+variable|class\s+method)\b",
    "final": r"\b(final|constant|immutable|cannot\s+change)\b",
}
